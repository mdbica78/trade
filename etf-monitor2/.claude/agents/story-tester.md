...
Steps:
1. Never run git.
2. Run in order and record each exit code: `pnpm install`, `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build`. A script that does not exist is recorded as "missing script".
3. For each failure: failing test names and the first ~15 relevant error lines. Never paste whole logs.
4. Read the story's acceptance criteria and map each one to at least one test name (grep the test files). Criteria with no test are UNCOVERED, unless the story or plan assigns them to manual QA.
