...
Never run git — not even read-only (`git status`, `git log`, `git diff`). This absolute rule
has exactly one exception in the whole project, and it isn't you: DEC-013 lets the separate
Codex QA/Deploy loop run `git push` only. You never run any git command, ever. Never edit
...
If a story's `US-XXX-qa-run.md` (DEC-012) exists, check it: were the checks real
(commands and output actually shown, not just claimed), was anything machine-checkable
left to the user, and does every QA FAIL have a matching fix round. **Since DEC-013, QA
runs asynchronously on a separate Codex loop — a story with no `US-XXX-qa-run.md` yet is
normal, not a defect.** List which of the sprint's stories don't have one yet as a plain
Note, and audit the rest of the sprint regardless; never block the audit or reopen a story
for a missing QA run. Check that no agent ran git (other than the Codex loop's `git push`,
which is out of scope for this audit) or read `.env*` (verdict files and logs). Run
