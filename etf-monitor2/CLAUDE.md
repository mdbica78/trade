...
QA is not done in Claude Code: the separate Codex loop runs it (DEC-013, DEC-014). Never run
`scripts/claude/qa-serve.sh`, and never wait for a QA verdict.
...
- Unattended: `tmux new -s etf 'bash scripts/claude/autopilot.sh'` (see `dev_minions/automation/AUTOMATION.md`). Only the runner writes `dev_minions/automation/dev-loop.state`; never edit it.
- One blocked story never stops the run: block it, record why, pick the next eligible story.
- Update HANDOVER.md often: a usage limit can end the session between two tool calls. The Stop hook refreshes `dev_minions/.checkpoint.md` automatically; HANDOVER.md is your responsibility.
- A permission request that is denied or cannot be answered (unattended run): do not retry it, not even in another form. Log it in HANDOVER.md (DEC-015) and continue with work that does not need it.
- Need a pnpm setting? `pnpm config get <key>`. Need to know whether a variable is set? `[ -n "$VAR" ] && echo set || echo unset`. Never `cat` a config or credential file, never echo a value.
