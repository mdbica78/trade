...
## Model and effort policy (DEC-005) — lowest cost that keeps quality
| Work | Who | Model | Effort |
|---|---|---|---|
| Orchestration + implementation | main session | sonnet | medium (settings.json) |
| Plan for a complex story; fix strategy after the 2nd FAIL | `story-planner` subagent | opus | high |
...
- Launch `story-reviewer` and `story-tester` in parallel, each with a short self-contained delegation prompt: story id, round number, paths to the story file and plan (the files changed are listed in HANDOVER.md). They start from a fresh context; that is what makes them independent. They read files themselves.

## Workflow
- Use the `deliver-story` skill for each story. Use the `handover` skill whenever you stop, for any reason.
- Never run git commands; the user handles git. Update HANDOVER.md often: a usage limit can end the session between two tool calls. The Stop hook refreshes `dev_minions/.checkpoint.md` (recently modified files) automatically; HANDOVER.md is your responsibility.
