...
OUT="dev_minions/.checkpoint.md"
{
  ...
  echo
  echo "## Files modified in the last 3 hours (excluding node_modules, .next, .git)"
  find . \( -path ./node_modules -o -path ./.next -o -path ./.git \) -prune -o \
       -type f -mmin -180 ! -name '.checkpoint.md' -printf '    %TY-%Tm-%Td %TH:%TM  %p\n' 2>/dev/null | sort -r | head -60
} > "$OUT" 2>/dev/null
exit 0
